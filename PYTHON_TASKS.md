...
Интерпретатор Python после установки располагается, обычно, по пути /usr/local/bin/python3.6 или /usr/local/bin/python2.7. Для проверки наличия интерпретатора в системе можно воспользоваться системной утилитами `whereis` или `which` (это верно  относительно UNIX систем).
Откройте командную строку и наберите команду python
    
    $ python

При этом у вас должна интерпретатор откроется для в вас в интерактивном режиме например так:
...
Теперь вы можете работать непосредственно с интерпретатором вводя команды. Чтобы завершить работу интерактивной оболочки Pyhton можно нажать клавиши <kbd>Ctrl</kbd> + <kbd>D</kbd> или  <kbd>Ctrl</kbd> + <kbd>C</kbd> также можно воспользоваться командой `quit()`.
